...
function renderEntries(data: Record<string, unknown> | null) {
  ...
  if (entries.length === 0) {
    return <p className="text-center text-sm text-muted-foreground">No support details available yet.</p>;
  }
...
export function Support() {
  const { supportData, loading, error } = useSupportPage();

  return (
    <>
      <Helmet>
        <title>AutoHub — Support</title>
      </Helmet>
        ...
        >
          <Card className="mx-auto w-full max-w-2xl border p-8 shadow-xs sm:p-10">
            {loading ? (
              ...
              <div className={cn('space-y-1 text-center sm:text-start')}>{renderEntries(supportData)}</div>
            )}
          </Card>
        </motion.div>
      </div>
